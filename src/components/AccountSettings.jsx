...
import { updateUserName, getUserEnquiries, getWishlist, removeFromWishlist } from '../firebase/users';
import styles from './AccountSettings.module.css';

const WA_NUMBER = '919997961188';

export default function AccountSettings({ user, onClose, onNameUpdate, onAddToCart, wishlist, onToggleWishlist, initialTab }) {
    ...
    <div className={styles.overlay} onClick={e => e.target === e.currentTarget && onClose()}>
      <div className={styles.modal}>
        ...
        <div className={styles.tabs}>
          ...
          <button className={`${styles.tab} ${tab === 'wishlist' ? styles.tabActive : ''}`} onClick={() => setTab('wishlist')}>
            ❤️ Wishlist
          </button>
        ...
        <div className={styles.content}>
          ...
          {tab === 'wishlist'  && <WishlistTab uid={user.uid} wishlist={wishlist} onToggleWishlist={onToggleWishlist} onAddToCart={onAddToCart} onClose={onClose} />}
...
function SetPasswordForm() {
  ...
  async function handleSubmit(e) {
    ...
    try {
      ...
      if (err.code === 'auth/requires-recent-login') setError('Please sign out, sign in again, and try immediately after.');
      else if (err.code === 'auth/credential-already-in-use') setError('A password is already set for this account.');
  ...
  return (
    <form onSubmit={handleSubmit} className={styles.form}>
      <p className={styles.hint}>Add a password so you can sign in from any device without needing your Google account.</p>
      {error   && <div className={styles.error}>{error}</div>}
      {success && <div className={styles.success}>✓ Password set successfully!</div>}
      <label className={styles.label}>New Password</label>
      <input className={styles.input} type="password" value={password} onChange={e => setPassword(e.target.value)} placeholder="At least 6 characters" autoComplete="new-password" />
      <label className={styles.label}>Confirm Password</label>
      <input className={styles.input} type="password" value={confirmPassword} onChange={e => setConfirmPassword(e.target.value)} placeholder="Re-enter password" autoComplete="new-password" />
      <button type="submit" className={styles.saveBtn} disabled={loading}>
...
function ChangePasswordForm() {
  ...
  return (
    <form onSubmit={handleSubmit} className={styles.form}>
      ...
      <input className={styles.input} type="password" value={currentPassword} onChange={e => setCurrentPassword(e.target.value)} placeholder="Enter current password" autoComplete="current-password" />
      <label className={styles.label}>New Password</label>
      <input className={styles.input} type="password" value={newPassword} onChange={e => setNewPassword(e.target.value)} placeholder="At least 6 characters" autoComplete="new-password" />
      <label className={styles.label}>Confirm New Password</label>
      <input className={styles.input} type="password" value={confirmPassword} onChange={e => setConfirmPassword(e.target.value)} placeholder="Re-enter new password" autoComplete="new-password" />
      <button type="submit" className={styles.saveBtn} disabled={loading}>
...
function WishlistTab({ uid, wishlist, onToggleWishlist, onAddToCart, onClose }) {
  if (wishlist.length === 0) {
    return (
      <div className={styles.emptyState}>
        <div className={styles.emptyIcon}>🤍</div>
        <p>No items in your wishlist yet.</p>
        <p className={styles.emptySub}>Tap the ❤️ on any product to save it here.</p>
      </div>
    );
  }
      ...
      {wishlist.map(product => (
        <div key={product.id} className={styles.wishlistItem}>
          ...
          <div className={styles.wishlistInfo}>
            <div className={styles.wishlistName}>{product.name}</div>
            <div className={styles.wishlistBrand}>{product.brand}</div>
...
function EnquiriesTab({ uid }) {
  ...
  function handleNewEnquiry() {
    const msg = encodeURIComponent('Hi! I have an enquiry about your products.');
  ...
  return (
    <div>
      <button className={styles.newEnquiryBtn} onClick={handleNewEnquiry}>
        + New Enquiry
      </button>
